import type { BreathingStep } from "../components/meditation/BreathingGuide";

export type MeditationScenario = {
  id: string;
  title: string;
  category: string;
...
export const meditationScenarios: MeditationScenario[] = [
  {
    id: "morning-calm",
    title: "Morning Calm",
    category: "relaxation",
...
export function findMeditationScenario(title: string, category: string) {
  const normalizedTitle = title.trim().toLowerCase();
  const normalizedCategory = category.trim().toLowerCase();

  return (
    meditationScenarios.find((item) => item.title.trim().toLowerCase() === normalizedTitle) ??
    meditationScenarios.find((item) => item.category.trim().toLowerCase() === normalizedCategory) ??
    meditationScenarios[0]
  );
}
